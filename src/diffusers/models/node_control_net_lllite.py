...
def load_control_net_lllite_patch(path, cond_image, multiplier, num_steps, start_percent, end_percent):
    ...
    import safetensors
    ctrl_sd = safetensors.torch.load_file(path)
...
class LLLiteLoader:
    ...
    def load_lllite(self, model, model_name, cond_image, strength, steps, start_percent, end_percent):
        ...
        if patch is not None:
            ...
            model_lllite.set_attn_processor(patch)
...
if __name__ == '__main__':
    ...
    cond_image = diffusers.utils.load_image("https://dev-nafo-legion-s3-creationsbucket070e58c8-pwxexcqh1mil.s3.eu-central-1.amazonaws.com/control_lineart_1024.png")
    # cond_image: PIL.Image to torch.Tensor
    cond_image = torch.tensor(cond_image)
    # cond_image: 3,h,w to b,h,w,3
    cond_image = cond_image.unsqueeze(0).permute(0, 3, 2, 1).permute(0, 2, 1, 3)

    controlnet = controlnet.load_lllite(model=pipe.unet, model_name="/notebooks/stable-diffusion-webui/models/ControlNet/kohya_controllllite_xl_canny.safetensors", cond_image=cond_image, strength=1.0, steps=20, start_percent=0.0, end_percent=100.0)
    print(controlnet)